...
dir <- setwd("/Users/julian/Documents/github/juliandefreitas/self/memory_distinct_selves/e1_memory_alternatives/data")
...
p1.111<-p1.11+scale_fill_discrete(name = "", labels = c ("Future\nYou1", "Stranger\nJohn", "Stranger\nBill")) +
  xlab ("") + ylab ("") +
...
p1.112<-p1.12+scale_fill_discrete(name = "", labels = c ("Future\nYou2", "Stranger\nJohn", "Stranger\nBill")) +
  xlab ("") + ylab ("") +
  theme(legend.text = element_text(size = "16", face = "plain")) +
  geom_errorbar(aes(ymax=mean+sem, ymin=mean-sem), position="dodge")+
  geom_signif(data=d.one,
...
p1.14<-ggplot(d.two,aes(x=factor(cond),y=mean,fill=factor(cond))) +  
  ...
  theme(axis.ticks.y = element_blank())+
  theme_classic()
p1.114<-p1.14+scale_fill_discrete(name = "", labels = c ("Future\nYou1", "Future\nYou2", "Stranger\nJohn")) +
  xlab ("") + ylab ("") +
  theme(legend.text = element_text(size = "16", face = "plain")) +
  geom_errorbar(aes(ymax=mean+sem, ymin=mean-sem), position="dodge")+
  geom_signif(data=d.two,
              aes(xmin=1, xmax=2, annotations=star_mat[3], y_position=0.68),
              textsize = 6, vjust = -0.4,
              manual=TRUE) +
  ggtitle ("Two Selves") +
  theme(plot.title = element_text(hjust = 0.5)) + 
...
p1.115<-p1.15+scale_fill_discrete(name = "", labels = c ("Original", "Copy", "Stranger")) +
  ...
  geom_signif(data=perf_mat_plot,
              aes(xmin=1, xmax=3, annotations=star_mat[4], y_position=1.9),
              textsize = 6, vjust = -0.4,
              manual=TRUE) +
  geom_signif(data=perf_mat_plot,
              aes(xmin=2, xmax=3, annotations=star_mat[5], y_position=1.7),
              textsize = 6, vjust = -0.4,
              manual=TRUE) +
...
annotate_figure(figure,top = text_grob("Experiment 1: Alternate Future Selves", face = "bold", size = 27),
