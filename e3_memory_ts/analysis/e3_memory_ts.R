...
for(i in 1:length(agentCond)) {
  if(condNum[i] == 1) {
    ...
    else if(agentCond[i] == 'surface-you') {
      data$agentCond_n[i] = 2
    }
    else if(agentCond[i] == 'stranger-john') {
      data$agentCond_n[i] = 3
...
p1.114<-p1.14+scale_fill_discrete(name = "", labels = c ("True\nYou", "Surface\nYou", "Stranger\nJohn")) +
  ...
  geom_signif(data=d.two,
              aes(xmin=1, xmax=2, annotations=star_mat[3], y_position=0.70),
              textsize = 8, vjust = 0.3,
              manual=TRUE) +
  ggtitle ("Two Selves") +
  theme(plot.title = element_text(hjust = 0.5)) + 
...
annotate_figure(figure,top = text_grob("Experiment 3: True and Surface Selves", face = "bold", size = 27),
